...
export default function LogModal({ name, token, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-gray-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-3xl flex flex-col max-h-[80vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 text-xs">
              <button onClick={() => setTab('out')} className={`px-3 py-1 ${tab === 'out' ? 'bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                stdout ({logs.out?.length || 0})
              </button>
              <button onClick={() => setTab('err')} className={`px-3 py-1 ${tab === 'err' ? 'bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
            ...
            <button
              ...
              className="text-xs px-2.5 py-1 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-slate-400 dark:hover:border-slate-500 disabled:opacity-50 transition-colors"
            >
        ...
        <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-slate-500 dark:text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <input
              ...
              className="flex-1 bg-transparent text-xs text-slate-700 dark:text-slate-300 placeholder-slate-500 dark:placeholder-slate-600 focus:outline-none"
            />
            {search && (
              <button onClick={() => setSearch('')} className="text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-xs">
                ✕ <span className="text-slate-500 dark:text-slate-600">{lines.length}/{allLines.length}</span>
              </button>
            )}
          </div>
        </div>
        ...
        {/* Log body */}
        <div ref={containerRef} className="flex-1 overflow-y-auto overscroll-contain p-4 font-mono text-xs bg-slate-200 dark:bg-slate-950 rounded-b-2xl">
          {loading ? (
            <div className="text-slate-600 dark:text-slate-400 text-center py-8">Loading logs…</div>
          ) : lines.length === 0 ? (
            <div className="text-slate-500 dark:text-slate-500 text-center py-8">No log lines found.</div>
          ) : (
...
function LogLine({ line, isErr }) {
  ...
  const color = isError ? 'text-red-400' : isWarn ? 'text-yellow-400' : isInfo ? 'text-blue-400' : 'text-slate-700 dark:text-slate-300';
