      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">All processes by memory</p>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                By Process {data?.processes && <span className="text-slate-500 normal-case font-normal">({data.processes.length} total)</span>}
              </div>
            </div>
            <input
              type="text"
              ...
              className="w-full mb-3 px-3 py-1.5 text-sm bg-slate-800 border border-slate-700 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-slate-500"
            />
            {loading ? (
...
function KillDialog({ name, pids, onConfirm, onCancel }) {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-red-800/60 rounded-2xl p-5 w-full max-w-sm shadow-2xl"
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-white font-semibold">Force Kill Process</h3>
        </div>
        <p className="text-slate-400 text-sm mb-1">
          Kill <span className="text-white font-semibold">{name}</span>?
        </p>
        <p className="text-xs text-slate-500 mb-1">
          {pids.length === 1 ? `PID ${pids[0]}` : `${pids.length} PIDs: ${pids.slice(0, 5).join(', ')}${pids.length > 5 ? '…' : ''}`}
        </p>
        <p className="text-xs text-slate-600 mb-4">This will send SIGKILL. Any unsaved state will be lost.</p>
        {error && <p className="text-xs text-red-400 mb-3">{error}</p>}
        <div className="flex gap-2 justify-end">
          <button
            onClick={onCancel}
            disabled={killing}
            className="px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-600 bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
...
function ProcessRow({ proc, total, onKill }) {
  ...
  return (
    <div>
      <div
        className={`flex items-center gap-3 ${canExpand ? 'cursor-pointer hover:bg-slate-800/50 rounded-lg px-1 -mx-1' : ''}`}
      ...
      {expanded && (
        <div className="ml-4 mt-1 mb-1 space-y-1 border-l-2 border-slate-700 pl-3">
          {proc.items.sort((a, b) => b.rss - a.rss).map(item => (
            <div key={item.pid} className="flex items-center gap-3 py-0.5">
              <div className="w-28 shrink-0">
                <div className="text-xs text-slate-400 font-mono">PID {item.pid}</div>
                <div className="text-xs text-slate-600 truncate">{item.cmd}</div>
              </div>
              <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span className="text-xs text-slate-400 w-14 text-right">{formatBytes(item.rss)}</span>
                <button
